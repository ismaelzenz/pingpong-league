...
to get an app icon and a standalone (no browser chrome) experience. Wiring lives in
`src/app/manifest.ts`, `public/sw.js` (service worker, registered by `src/components/ServiceWorker.tsx`),
and the icon/theme metadata in `src/app/layout.tsx`. App icons are generated by
`npm run gen:icons` (pure Node, no deps) into `public/`.
